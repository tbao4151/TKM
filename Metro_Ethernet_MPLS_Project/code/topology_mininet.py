...
from network_config import configure_network, connectivity_checks
...
class MetroEthernetTopo(Topo):
    def build(self):
        ...
        # Branch 2: Three-tier Network - Core/Distribution/Access redundancy.
        ...
        for acc in ("b2_acc1", "b2_acc2", "b2_acc3"):
            for dist in ("b2_dist1", "b2_dist2"):
                self.addLink(acc, dist, cls=TCLink, bw=100, delay="1ms")
        for dist in ("b2_dist1", "b2_dist2"):
            for core in ("b2_core1", "b2_core2"):
                self.addLink(dist, core, cls=TCLink, bw=100, delay="1ms")
        self.addLink("b2_core1", "b2_core2", cls=TCLink, bw=100, delay="1ms")
        self.addLink("b2_core1", "b2_ce", cls=TCLink, bw=100, delay="1ms")

        # Branch 3: Leaf-Spine Network - 2 spine, 4 leaf, 7 endpoint.
        ...
        for leaf in ("b3_leaf1", "b3_leaf2", "b3_leaf3", "b3_leaf4"):
            for spine in ("b3_spine1", "b3_spine2"):
                self.addLink(leaf, spine, cls=TCLink, bw=100, delay="1ms")
        ...
        self.addLink("p1", "p3", cls=TCLink, bw=50, delay="4ms")
        self.addLink("p2", "p4", cls=TCLink, bw=50, delay="4ms")
        self.addLink("p3", "p4", cls=TCLink, bw=50, delay="4ms")
        self.addLink("p1", "p4", cls=TCLink, bw=50, delay="4ms")
        self.addLink("p2", "p3", cls=TCLink, bw=50, delay="4ms")
...
def start_configured_network() -> Mininet:
    net = build_network()
    net.start()
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Metro Ethernet MPLS-like Mininet topology")
    ...
    try:
        ...
        info("*** Topology ready. Use Mininet CLI for demo.\n")
